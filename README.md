...
The probe must provide `GET /api/lights/{id}/snapshot`, version 1, including a stable `planId`, `serverTimeMillis`, `effectiveStartMillis`, `validFromMillis` and `validUntilMillis`, source capabilities, protocol status and the complete decoded schedule. The options flow additionally reads `GET /api/catalog` (`{"lights": [{"id", "name", "label", "paired", "minKelvin", "maxKelvin"}]}`) to list the probe's lights; a probe without that endpoint only loses the list. Use the updated `apple-adaptive-lighting-collector` supplied alongside this integration.
