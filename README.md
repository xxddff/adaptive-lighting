...
- [Apple Adaptive Lighting curves](#apple-adaptive-lighting-curves)
  - [Brightness and manual control](#brightness-and-manual-control)
  - [Cache, retries and fallback](#cache-retries-and-fallback)
...
|:--------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:---------------|:----------------------------------------|
| `color_source`                              | Use the sun algorithm or a captured Apple color-temperature plan.                                                                                                                                                                                                                                                                                                                           | `sun`          | one of `('sun', 'apple')`               |
| `apple_probe_url`                           | Apple Curve Probe base URL, for example http://192.168.1.50:8787.                                                                                                                                                                                                                                                                                                                           | ``             | `str`                                   |
| `apple_curve_id`                            | ID of the probe's virtual light whose Apple plan should control these lights (1-32 lowercase letters, digits or hyphens). The UI lists the probe's lights to choose from.                                                                                                                                                                                                                   | ``             | `str`                                   |
...
## Apple Adaptive Lighting curves

Set `color_source` to `apple` to use a captured Apple color-temperature plan while retaining Adaptive Lighting's brightness algorithm, transitions and sleep mode. The default `sun` preserves existing behavior.

In the integration options, choose `apple` and enter the Apple Curve Probe base URL. Home Assistant then asks the probe (`GET /api/catalog`) which virtual lights exist and lists them by name, ID, Kelvin range and pairing state, so you can pick the one whose plan should drive these lights. The dropdown also accepts a typed ID, for a light you have not created on the probe yet. If the probe cannot be reached, a menu lets you correct the address or enter the ID by hand instead; saving never requires the probe to be online. There is no automatic matching between the probe's lights and your Home Assistant lights.

The ID is the identifier entered when creating the virtual light on the probe: 1–32 lowercase letters, digits or hyphens. Lights migrated from probe 0.2.0 keep the IDs `yeelight`, `ikea-zigbee` and `ikea-matter`. The URL must be reachable **from Home Assistant**, for example `http://192.168.1.50:8787`, without `/api/...`. `localhost` points to Home Assistant's own host or container.
...
adaptive_lighting:
  - name: Living room
    lights:
      - light.living_room
    color_source: apple
    apple_probe_url: http://192.168.1.50:8787
    apple_curve_id: living-room
```

The three source settings are available through UI/YAML and take effect on integration reload; they are not accepted by `adaptive_lighting.change_switch_settings`. With `color_source: apple`, both `apple_probe_url` and `apple_curve_id` are required; YAML does not query the probe.

### Brightness and manual control

Apple's brightness adjustment factor modifies **color temperature**, not brightness. Each light uses its own effective brightness: an explicit brightness in the command, the adaptive brightness being sent, or its latest known brightness. Calculation interpolates the base temperature and factor in mired, then converts to Kelvin. The plan's minimum brightness for this calculation does not raise the light's brightness.

With manual takeover enabled, Apple profiles use `pause_changed`, including while temporarily falling back to sun: manually changing brightness preserves your chosen brightness while color continues adapting; manually changing color pauses color adaptation. Brightness that a light reports while it is still executing an Adaptive Lighting transition is not a manual change; its color is re-evaluated once that transition has ended. Sleep mode, the brightness/color switches and existing reset rules remain in effect. Setting `take_over_control: false` still disables manual takeover. Source changes do not reset manual-control flags or turn on lights that are off.

The existing `interval` and transition settings control light updates independently of fetching plans. Group-level color attributes are `null` while using Apple because lights with different brightness can have different colors. No additional state entities, status attributes or dashboard UI are created.

### Cache, retries and fallback

The probe must provide `GET /api/lights/{id}/snapshot`, version 1, including a stable `planId`, `serverTimeMillis`, `effectiveStartMillis`, `validFromMillis` and `validUntilMillis`, source capabilities, protocol status and the complete decoded schedule. The options flow additionally reads `GET /api/catalog` (`{"lights": [{"id", "name", "label", "paired", "minKelvin", "maxKelvin"}]}`) to list the probe's lights; a probe without that endpoint only loses the list. Use the `apple-adaptive-lighting-collector` (Apple Curve Probe) supplied alongside this integration.

- Valid plans are cached on disk and executed locally. **There is no polling while the cached plan is valid.** Plans received early by the probe, or disabled in Apple Home, are therefore not observed until the next fetch.
- At expiry, Home Assistant fetches immediately. While no current valid plan is available, it retries every **60 seconds**.
- For **30 minutes after the original expiry**, a light uses the old plan's final base temperature and brightness factor. Brightness adjustments continue to affect its color.
- After that grace period, color smoothly falls back to the configured sun algorithm. With no prior usable plan, fallback starts immediately. An explicit disabled response revokes the plan immediately without a grace period.
- A future plan waits for its start time. A valid replacement resumes Apple behavior using the configured transition.
- Duplicate old plans, fetch failures and restarts never restart the validity or grace window. Changing source address or ID selects a separate cache.

Fallback creates one Home Assistant persistent notification per shared probe URL/curve ID, listing affected profiles. Recovery dismisses it. Disabling or removing a profile removes it from that notification; deliberate selection of `sun` is not an error. There are no phone notifications.

The source, cache and requests are shared by profiles using the same normalized URL and ID. Turning off the last profile stops its timers and requests; turning it back on first restores its cache.
