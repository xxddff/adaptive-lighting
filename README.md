...
| `apple_curve_id`                            | Virtual light ID whose Apple plan should control these lights.                                                                                                                                                                                                                                                                                                                              | `yeelight`     | one of `('yeelight', 'ikea-zigbee', 'ikea-matter')` |
...
In the integration options, choose `apple`, then enter the Apple Curve Probe base URL and select `yeelight`, `ikea-zigbee` or `ikea-matter`. Choose the ID yourself; there is no automatic matching. The URL must be reachable **from Home Assistant**, for example `http://192.168.1.50:8787`, without `/api/...`. `localhost` points to Home Assistant's own host or container. An offline probe does not prevent saving these options.
...
adaptive_lighting:
  - name: Living room
    ...
    apple_curve_id: yeelight
```

The three source settings are available through UI/YAML and take effect on integration reload; they are not accepted by `adaptive_lighting.change_switch_settings`.
...
The probe must provide `GET /api/lights/{id}/snapshot`, version 1, including a stable `planId`, `serverTimeMillis`, `effectiveStartMillis`, `validFromMillis` and `validUntilMillis`, source capabilities, protocol status and the complete decoded schedule. Use the `apple-adaptive-lighting-collector` (Apple Curve Probe) supplied alongside this integration.
