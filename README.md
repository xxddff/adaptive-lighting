...
The probe must provide `GET /api/lights/{id}/snapshot`, version 1, including a stable `planId`, `serverTimeMillis`, `effectiveStartMillis`, `validFromMillis` and `validUntilMillis`, source capabilities, protocol status and the complete decoded schedule. Use the updated `apple-home-curve-probe` supplied alongside this integration.
